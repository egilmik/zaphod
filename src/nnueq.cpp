...
// Generated by cmake/ZaphodNnue.cmake, tells us which network was configured.
#include "zaphod_nnue_embed.h"

using namespace NNUE;

// Number of bytes loadEmbedded() reads out of the network blob. The file may
// be larger, trainers pad the tail out to an alignment boundary.
static constexpr std::size_t NETWORK_BYTES = sizeof(Network::l0w) + sizeof(Network::l0b)
+ sizeof(Network::l1w) + sizeof(Network::l1b);

alignas(32) static const unsigned char embeddedNetwork[] = {
    #embed ZAPHOD_NNUE_FILE
};

static_assert(sizeof(embeddedNetwork) == ZAPHOD_NNUE_FILE_BYTES,
    "embedded network differs from the file CMake configured");
static_assert(sizeof(embeddedNetwork) >= NETWORK_BYTES,
    "embedded network is too small for the architecture in nnueq.h");
...
bool NNUEQ::loadEmbedded() {
    ...
    auto take = [&](void* dst, size_t bytes) {
        std::memcpy(dst, embeddedNetwork + off, bytes);
        off += bytes;
        };
