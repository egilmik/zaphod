...
int Search::alphaBetaMax(Board board, int alpha, int beta, int depth)
{
    if(depth == 0){
...
int Search::alphaBetaMin(Board board, int alpha, int beta, int depth)
{
...
int Search::negaMax(Board board, int alpha, int beta, int depthLeft)
{
    ...
    for(int i = 0; i < moveList.counter; i++){
        ...
        bool valid = board.makeMove(move.fromSq,move.toSq,move.piece,move.capture,move.enpassant,move.doublePawnPush,move.castling,move.promotion);
        if(valid){
            score = -negaMax(board,-alpha,-beta,depthLeft-1);
        }
        if(score >= beta){
            return beta;
        }
