...
int Search::negamax(Board& board, int depth, int alpha, int beta, int ply, bool pvNode)
{
    ...
    int correction = history.correction(board.getSideToMove(),board.getKeyHistory(),board.getHistoryPly());
...
int Search::qsearch(Board &board, int alpha, int beta,int depth, int ply, bool pvNode)
{
    ...
    if (inCheck) {
        ...
        ss[ply].staticEval = std::clamp(ss[ply].rawStaticEval + history.correction(board.getSideToMove(), board.getKeyHistory(), board.getHistoryPly()), -MATE_IN_MAX + 1, MATE_IN_MAX - 1);
