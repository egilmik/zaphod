...
void UCI::bench() {
    ...
    for (std::string fen : fenList) {
        ...
        nodes += search.evaluatedNodes;

        auto stop = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
        //duration + 1 to prevent divide by zero
        int nps = nodes / (duration.count()+1) / 1000;
        
        std::cout << "Score: " << (double)move.score / 100.0 << " Depth: ";
    ...
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
    int nps = (double)nodes / ((double)duration.count() / (double)1000);
