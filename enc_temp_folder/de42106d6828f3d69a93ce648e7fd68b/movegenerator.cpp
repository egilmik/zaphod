...
void MoveGenerator::generateMoves(Board &board,MoveList &moveList)
{
    MoveGenerator gen;
    History hist;
    gen.init(board, Move{}, false, &hist, nullptr);
...
void MoveGenerator::init(Board& b, Move tt, bool onlyCaptures, History *hist, History::ContSlice* const* cont) {
...
void MoveGenerator::scoreQuietMoves() {
    ...
    for (int i = 0; i < quietCount; i++) {
        ...
        if(contSlices){            
            ...
            quietMoves[i].score += history->contScore(contSlices, piece, quietMoves[i].move.to(), 3) * movegenContWeight6Ply() / 100;
        }	       
    }
