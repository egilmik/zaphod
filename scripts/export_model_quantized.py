...
# Dims
IN_FEATS = 768
HIDDEN   = sd["l1.weight"].shape[0]  # should be 32
assert HIDDEN == 128, f"Unexpected HIDDEN={HIDDEN}"

# Float weights
w1 = sd["l1.weight"].cpu().numpy().astype(np.float32)   # (H, IN)
b1 = sd["l1.bias"  ].cpu().numpy().astype(np.float32)   # (H,)
...
print(f"wrote {BIN_OUT}  (H={HIDDEN}, a1={a1:.6g}, Q_CAP={Q_CAP}, s2={s2:.6g}, eval_scale={eval_scale})")
